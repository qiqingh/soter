    if [ "`syscfg get hardware_vendor_name`" = "Broadcom" ] ; then
        ...
        if [ "$IS_PINNACLES" = "1" ] ; then       
            if [ -n "$lan_ports" ] ; then
                ...
                if [ -n "$port_switch_id" ] ; then
                    j=1
                    for i in $port_switch_id; do
                        port_no=$(echo $lan_ports | cut -f $j -d ' ')
                        if [ $i = 0 ]; then
                            internal_switch_port="$port_no $internal_switch_port"
                        elif [ $i = 1 ]; then
                            external_switch_port="$port_no $external_switch_port"
                        fi
                        j=`expr $j + 1`
                    done
                    lan_ports=$internal_switch_port
                fi
            ...
            if [ -n "$external_switch_port" ] ; then
                for i in $external_switch_port; do
                    et -i eth0 ephywr 0x$i 0x00 0x1940
                done
                sleep 1.5
                for i in $external_switch_port; do
                    et -i eth0 ephywr 0x$i 0x00 0x1140
                done
            fi
        else
            if [ -n "$lan_ports" ] ; then
