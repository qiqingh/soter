    ...
    wan_prio=`syscfg get wan_1::prio`
    if [ "`syscfg get vlan_tagging::enabled`" = "1" -a "$2" != "1" -a -n "$wan_prio" -a "$wan_prio" != "0" ] ; then
        vconfig set_egress_map vlan$2 0 $wan_prio
        vconfig set_egress_map vlan$2 1 $wan_prio
        vconfig set_egress_map vlan$2 2 $wan_prio
        vconfig set_egress_map vlan$2 3 $wan_prio
        vconfig set_egress_map vlan$2 4 $wan_prio
        vconfig set_egress_map vlan$2 5 $wan_prio
        vconfig set_egress_map vlan$2 6 $wan_prio
        vconfig set_egress_map vlan$2 7 $wan_prio
    fi
