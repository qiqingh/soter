   ...
   if [ ! -e $CRONTAB_FILE ] ; then
      ...
      if [ "1" != "`syscfg get ipv6::passthrough_done_in_hw`" ] ; then
          echo "#! /bin/sh" > /etc/cron/cron.everyminute/ipv6_passthrough_monitor_everyminute.sh
          echo "/etc/init.d/ipv6_passthrough_monitor.sh" >> /etc/cron/cron.everyminute/ipv6_passthrough_monitor_everyminute.sh
          chmod 700 /etc/cron/cron.everyminute/ipv6_passthrough_monitor_everyminute.sh
      fi
	  if [ "true" = "$WIFI_STA" ]; then
