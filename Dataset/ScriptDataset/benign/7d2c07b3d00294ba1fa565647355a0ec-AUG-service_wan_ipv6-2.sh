   echo "register_ipv6_handlers: entry; IPV6_WAN_PROTO=$IPV6_WAN_PROTO" >> $LOG
   ...
   case "$IPV6_WAN_PROTO" in
      ...
      passthrough)
         uninstall_hw_nat
         ulog ipv6 status "$PID installing handlers for passthrough_link and passthrough_wan"
         asyncid=`sysevent async phylink_wan_state /etc/init.d/service_wan_ipv6/passthrough_link.sh`
         sysevent set ${SERVICE_NAME}_phylink_wan_state_asyncid "$asyncid"
         asyncid=`sysevent async desired_ipv6_link_state /etc/init.d/service_wan_ipv6/passthrough_link.sh`
         sysevent set ${SERVICE_NAME}_desired_ipv6_link_state_asyncid "$asyncid"
         asyncid=`sysevent async lan-status /etc/init.d/service_wan_ipv6/passthrough_link.sh`
         sysevent set ${SERVICE_NAME}_lan-status_asyncid "$asyncid"
         asyncid=`sysevent async guest_access-status /etc/init.d/service_wan_ipv6/passthrough_link.sh`
         sysevent set ${SERVICE_NAME}_guest_access-status_asyncid "$asyncid"
         asyncid=`sysevent async desired_ipv6_wan_state /etc/init.d/service_wan_ipv6/passthrough_wan.sh`
         sysevent set ${SERVICE_NAME}_desired_ipv6_wan_state_asyncid "$asyncid"
         asyncid=`sysevent async current_ipv6_link_state /etc/init.d/service_wan_ipv6/passthrough_wan.sh`
         sysevent set ${SERVICE_NAME}_current_ipv6_link_state_asyncid "$asyncid"
         ;;
      6rd)
