    ...
    if [ "$product" = "savannah" -o "$product" = "athens" -o "$product" = "atlanta" -o "$product" = "augusta" -o "$product" = "cocoon" -o "$product" = "austin" -o "$product" = "waldorf" ]; then
        prio=`syscfg get wan_1::prio`
        vconfig set_egress_map vlan$2 0 $prio
        vconfig set_egress_map vlan$2 1 $prio 
        vconfig set_egress_map vlan$2 2 $prio 
        vconfig set_egress_map vlan$2 3 $prio 
        vconfig set_egress_map vlan$2 4 $prio 
        vconfig set_egress_map vlan$2 5 $prio 
        vconfig set_egress_map vlan$2 6 $prio 
        vconfig set_egress_map vlan$2 7 $prio 
    fi
