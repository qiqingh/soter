	...
	*)
	...
	if [ "EA9200" = "$MODEL_NAME" ] || [ "EA9500" = "$MODEL_NAME" ] || [ "EA6350" = "$MODEL_NAME" ] ; then
		...
		if [ "enabled" = "`syscfg get "$WL_SYSCFG"_pmf`" ] ; then
			...
			if [ "wpa2-personal" = "$SYSCFG_SECURITY_MODE" ] || [ "wpa-mixed" = "$SYSCFG_SECURITY_MODE" ] || [ "wpa2-enterprise" = "$SYSCFG_SECURITY_MODE" ] || [ "wpa-enterprise-mixed" = "$SYSCFG_SECURITY_MODE" ]; then	
				wl -i $USER_VAP mfp_config 1
				if [ "EA6350" = "$MODEL_NAME" ] ; then
					wl -i $USER_VAP mfp_sha256 0
				fi
			fi
		fi
	fi
