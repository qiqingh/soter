	...
	STATE_BSD=`syscfg get wl2_state`
	...
	if [ "up" = "$STATE_24" ] || [ "up" = "$STATE_5" ] || [ "up" = "$STATE_BSD" ]; then
		...
		if [ "up" = "$STATE_BSD" ] && [ "wep" != "`syscfg get wl2_security_mode`" ]; then
			BAND_STEERING_INT=`syscfg get wifi::band_steering_int`
			NAS_EAPD_IF_LIST=`echo "$NAS_EAPD_IF_LIST $BAND_STEERING_INT"`
		fi
	...
	for PHY_IF in $PHYSICAL_IF_LIST; do
		...
		elif [ "wl2" = "$WL_SYSCFG" ]; then
			WL2_WPS_STATE=`sysevent get "$WL_SYSCFG"_wps_status`
		fi
		DEF_SSID=`syscfg get wl_default_ssid`
		if [ "unconfigured" = "$WL0_WPS_STATE" ] || [ "unconfigured" = "$WL1_WPS_STATE" ] || [ "unconfigured" = "$WL2_WPS_STATE" ]; then
		...
		elif [ "configured" = "$WL0_WPS_STATE" ] || [ "configured" = "$WL1_WPS_STATE" ] || [ "configured" = "$WL2_WPS_STATE" ]; then
	...
	if [ "disabled" != "$WL0_WPS_STATE" ] || [ "disabled" != "$WL1_WPS_STATE" ] || [ "unconfigured" = "$WL2_WPS_STATE" ] || [ "configured" = "$WL2_WPS_STATE" ] ; then
		initialize_wps
		...
		if [ "1" = "`syscfg get wifi::band_steering_configure`" ] && [ "disabled" != "$WL2_WPS_STATE" ] && [ "up" = "$STATE_BSD" ]; then
			BAND_STEERING_INT=`syscfg get wifi::band_steering_int`
			WPS_EAPD_IF_LIST=`echo "$WPS_EAPD_IF_LIST $BAND_STEERING_INT"`
		fi
		if [ "1" = "`syscfg get wifi::band_steering_configure`" ] && [ "1" = "`syscfg get wifi::band_steering_enable`" ] ; then
			if [ "2" = "`syscfg get wifi::band_steering_mode`" ] && [ "down" != "`syscfg get wl0_state`" ] && [ "down" != "`syscfg get wl1_state`" ] && [ "down" != "`syscfg get wl2_state`" ] && [ "`syscfg get wl1_ssid`" = "`syscfg get wl0_ssid`" ] && [ "`syscfg get wl1_ssid`" = "`syscfg get wl2_ssid`" ]; then
				BSD_EAPD_IF_LIST="eth1 eth2 eth3"
				...
				BSD_EAPD_IF_LIST="eth1 eth3"
			fi
			/bin/eapd -nas $NAS_EAPD_IF_LIST -wps $WPS_EAPD_IF_LIST -bsd $BSD_EAPD_IF_LIST
		else
		...
		if [ "1" = "`syscfg get wifi::band_steering_configure`" ] && [ "1" = "`syscfg get wifi::band_steering_enable`" ] ; then
			...
			/bin/eapd -nas $NAS_EAPD_IF_LIST -bsd $BSD_EAPD_IF_LIST
		else
