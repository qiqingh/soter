...
if [ "1" = "`syscfg get guest_enabled`" -a "started" != "`sysevent get guest_access-status`" ]
then
   ulog default_ipv6_link status "$PID Deferring link up until guest_access-status is up."
   exit 0
fi
